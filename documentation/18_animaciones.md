...
| 14 | Press feedback estándar (`:active`) en botones/cards/filas | CSS puro | Global | ✅ Regla global `button:not(:disabled):active` y `[role=button]` con `transform: scale(.97)` en global.css (guard reduced-motion); variantes propias de Button/login/home card ganan por especificidad; `:active` + transition transform en los links-tarjeta del panel supervisor |
| 15 | Foco suave en campos de texto | CSS puro | Global | ✅ En touch no hay `:focus-visible` de teclado: los campos ahora muestran un anillo suave (`box-shadow 0 0 0 2px color-mix(primary 30%)`) en `:focus` (no solo focus-visible) con transición de `border-color`/`box-shadow`; redondeado con transición `none` bajo reduced-motion |
| 16 | Pop al alternar presente/ausente | CSS puro | Detalle escuela | ✅ La etiqueta de estado de cada fila se re-anima al cambiar (key por `presente`) con un micro pop (`attendance-label-pop`, scale 1→1.08→1) y colorea el veredicto verde/rojo (no color-only: el texto sigue diciendo Presente/Ausente). Guard reduced-motion |
| 17 | Tabs Hoy/Histórico con píldora deslizante + crossfade | CSS puro | Detalle escuela | ✅ La pestaña activa ya no pinta fondo propio: un indicador absoluto (`position:absolute`, acomodo por left/right sobre el content-box) se traslada `translateX(100%)` con transición de transform; contenido de cada vista entra con fade de 220ms (`.supervisor-detail__view`); roles tablist/tab/tabpanel + aria-selected. Guards de reduced-motion |
| 18 | Morph shared element hacia la hero desde GlobalSearch | View Transitions | GlobalSearch → detalle | ✅ Resultados que aterrizan en el detalle de una escuela (escuela y docente) adoptan `view-transition-name: school-hero` durante la navegación vía sub-componente `SearchResultItem` (+ `useViewTransitionState` con `navigate(to, {viewTransition})`). Historial no navega a detalle de escuela (solo vuelve al Home), así que el morph aplica únicamente desde GlobalSearch. Guard reduced-motion |
| 19 | Rebote en los contadores del resumen del día | CSS + useAmbientMotion | /supervisor | ✅ Componente `LiveCount`: el número se re-anima con un bump (key por valor) cada vez que el snapshot de Firestore agrega registros hoy. Gateado por `useAmbientMotion` para no animar en conexiones débiles/offline; `useReducedMotion` ya forma parte del hook. Solo transform/opacity |
| 20 | Destello de éxito al verificar asistencia | CSS puro | Detalle escuela (asistencias) | ✅ El badge "Verificada" se monta al confirmarse y entra con pop (`verify-badge-pop`: scale .8→1.06→1 + opacity) dejando un halo verde suave que se desvanece (box-shadow expandido→transparente). `display:inline-block` para que transform aplique. Guard reduced-motion |
